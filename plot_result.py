...
def plot_result(tensors: torch.tensor, save_path):
    ...
    width = cols * 1024 // 8 * 10 + (cols - 1) * 50
    height = rows * 1024 + (rows - 1) * 50
    ...
    plt.show()
    fig.savefig("assembled_tensors_with_colorbars.png", dpi=300, bbox_inches='tight', pad_inches=0)
    plt.close(fig)
