...
class RMILoss(nn.Module):
	...
	def forward(self, labels_4D, logits_4D, w_map, class_weight, epoch=None):
...
if __name__ == "__main__":
	mask = (torch.rand((1, 1, 128, 128)) > 0.5).to(torch.int)
	pred = torch.rand((1, 1, 128, 128))

	RMILossModel = RMILoss(num_classes=2)

	print(RMILossModel(mask, pred))