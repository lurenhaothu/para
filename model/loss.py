...
def BCELoss(mask, pred, map):
    return torch.mean(- mask * torch.log(pred + 1e-7) - (1 - mask) * torch.log(1 - pred + 1e-7))

def BCE_withClassBalance(mask, pred, map): #20570595.0 84287005.0
    count1 = 20570595
    count0 = 84287005
...
def Unet_Weight_BCE(mask, pred, map):
    count1 = 20570595
    count0 = 84287005
    w1 = (count1 + count0) / count1 / 2
    w0 = (count1 + count0) / count0 / 2
    return torch.mean(- (map + w1) * mask * torch.log(pred + 1e-7) - w0 * (1 - mask) * torch.log(1 - pred + 1e-7))
